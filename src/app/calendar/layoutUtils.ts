...
export function calculateCalendarLayout(days: Date[], tasks: Task[]): Record<string, CalendarEventSlot[]> {
    ...
    const sortedTasks = [...tasks].sort((a, b) => {
        // Internal (0) before External (1)
        const extA = (a as any).isExternal ? 1 : 0;
        const extB = (b as any).isExternal ? 1 : 0;
    ...
    sortedTasks.forEach(task => {
        ...
        days.forEach(d => {
            ...
            // Check if current day is within [start, end] range (inclusive)
